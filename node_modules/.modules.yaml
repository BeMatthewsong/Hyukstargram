hoistPattern:
  - '*'
hoistedDependencies:
  /@aashutoshrathi/word-wrap/1.2.6:
    '@aashutoshrathi/word-wrap': private
  /@ampproject/remapping/2.3.0:
    '@ampproject/remapping': private
  /@babel/code-frame/7.24.2:
    '@babel/code-frame': private
  /@babel/compat-data/7.24.4:
    '@babel/compat-data': private
  /@babel/core/7.24.4:
    '@babel/core': private
  /@babel/generator/7.24.4:
    '@babel/generator': private
  /@babel/helper-compilation-targets/7.23.6:
    '@babel/helper-compilation-targets': private
  /@babel/helper-environment-visitor/7.22.20:
    '@babel/helper-environment-visitor': private
  /@babel/helper-function-name/7.23.0:
    '@babel/helper-function-name': private
  /@babel/helper-hoist-variables/7.22.5:
    '@babel/helper-hoist-variables': private
  /@babel/helper-module-imports/7.24.3:
    '@babel/helper-module-imports': private
  /@babel/helper-module-transforms/7.23.3(@babel/core@7.24.4):
    '@babel/helper-module-transforms': private
  /@babel/helper-plugin-utils/7.24.0:
    '@babel/helper-plugin-utils': private
  /@babel/helper-simple-access/7.22.5:
    '@babel/helper-simple-access': private
  /@babel/helper-split-export-declaration/7.22.6:
    '@babel/helper-split-export-declaration': private
  /@babel/helper-string-parser/7.24.1:
    '@babel/helper-string-parser': private
  /@babel/helper-validator-identifier/7.22.20:
    '@babel/helper-validator-identifier': private
  /@babel/helper-validator-option/7.23.5:
    '@babel/helper-validator-option': private
  /@babel/helpers/7.24.4:
    '@babel/helpers': private
  /@babel/highlight/7.24.2:
    '@babel/highlight': private
  /@babel/parser/7.24.4:
    '@babel/parser': private
  /@babel/plugin-transform-react-jsx-self/7.24.1(@babel/core@7.24.4):
    '@babel/plugin-transform-react-jsx-self': private
  /@babel/plugin-transform-react-jsx-source/7.24.1(@babel/core@7.24.4):
    '@babel/plugin-transform-react-jsx-source': private
  /@babel/template/7.24.0:
    '@babel/template': private
  /@babel/traverse/7.24.1:
    '@babel/traverse': private
  /@babel/types/7.24.0:
    '@babel/types': private
  /@esbuild/aix-ppc64/0.20.2:
    '@esbuild/aix-ppc64': private
  /@esbuild/android-arm/0.20.2:
    '@esbuild/android-arm': private
  /@esbuild/android-arm64/0.20.2:
    '@esbuild/android-arm64': private
  /@esbuild/android-x64/0.20.2:
    '@esbuild/android-x64': private
  /@esbuild/darwin-arm64/0.20.2:
    '@esbuild/darwin-arm64': private
  /@esbuild/darwin-x64/0.20.2:
    '@esbuild/darwin-x64': private
  /@esbuild/freebsd-arm64/0.20.2:
    '@esbuild/freebsd-arm64': private
  /@esbuild/freebsd-x64/0.20.2:
    '@esbuild/freebsd-x64': private
  /@esbuild/linux-arm/0.20.2:
    '@esbuild/linux-arm': private
  /@esbuild/linux-arm64/0.20.2:
    '@esbuild/linux-arm64': private
  /@esbuild/linux-ia32/0.20.2:
    '@esbuild/linux-ia32': private
  /@esbuild/linux-loong64/0.20.2:
    '@esbuild/linux-loong64': private
  /@esbuild/linux-mips64el/0.20.2:
    '@esbuild/linux-mips64el': private
  /@esbuild/linux-ppc64/0.20.2:
    '@esbuild/linux-ppc64': private
  /@esbuild/linux-riscv64/0.20.2:
    '@esbuild/linux-riscv64': private
  /@esbuild/linux-s390x/0.20.2:
    '@esbuild/linux-s390x': private
  /@esbuild/linux-x64/0.20.2:
    '@esbuild/linux-x64': private
  /@esbuild/netbsd-x64/0.20.2:
    '@esbuild/netbsd-x64': private
  /@esbuild/openbsd-x64/0.20.2:
    '@esbuild/openbsd-x64': private
  /@esbuild/sunos-x64/0.20.2:
    '@esbuild/sunos-x64': private
  /@esbuild/win32-arm64/0.20.2:
    '@esbuild/win32-arm64': private
  /@esbuild/win32-ia32/0.20.2:
    '@esbuild/win32-ia32': private
  /@esbuild/win32-x64/0.20.2:
    '@esbuild/win32-x64': private
  /@eslint-community/eslint-utils/4.4.0(eslint@8.57.0):
    '@eslint-community/eslint-utils': public
  /@eslint-community/regexpp/4.10.0:
    '@eslint-community/regexpp': public
  /@eslint/eslintrc/2.1.4:
    '@eslint/eslintrc': public
  /@eslint/js/8.57.0:
    '@eslint/js': public
  /@fastify/busboy/2.1.1:
    '@fastify/busboy': private
  /@firebase/analytics-compat/0.2.8(@firebase/app-compat@0.2.31)(@firebase/app@0.10.1):
    '@firebase/analytics-compat': private
  /@firebase/analytics-types/0.8.1:
    '@firebase/analytics-types': private
  /@firebase/analytics/0.10.2(@firebase/app@0.10.1):
    '@firebase/analytics': private
  /@firebase/app-check-compat/0.3.10(@firebase/app-compat@0.2.31)(@firebase/app@0.10.1):
    '@firebase/app-check-compat': private
  /@firebase/app-check-interop-types/0.3.1:
    '@firebase/app-check-interop-types': private
  /@firebase/app-check-types/0.5.1:
    '@firebase/app-check-types': private
  /@firebase/app-check/0.8.3(@firebase/app@0.10.1):
    '@firebase/app-check': private
  /@firebase/app-compat/0.2.31:
    '@firebase/app-compat': private
  /@firebase/app-types/0.9.1:
    '@firebase/app-types': private
  /@firebase/app/0.10.1:
    '@firebase/app': private
  /@firebase/auth-compat/0.5.6(@firebase/app-compat@0.2.31)(@firebase/app-types@0.9.1)(@firebase/app@0.10.1):
    '@firebase/auth-compat': private
  /@firebase/auth-interop-types/0.2.2:
    '@firebase/auth-interop-types': private
  /@firebase/auth-types/0.12.1(@firebase/app-types@0.9.1)(@firebase/util@1.9.5):
    '@firebase/auth-types': private
  /@firebase/auth/1.7.1(@firebase/app@0.10.1):
    '@firebase/auth': private
  /@firebase/component/0.6.6:
    '@firebase/component': private
  /@firebase/database-compat/1.0.4:
    '@firebase/database-compat': private
  /@firebase/database-types/1.0.2:
    '@firebase/database-types': private
  /@firebase/database/1.0.4:
    '@firebase/database': private
  /@firebase/firestore-compat/0.3.29(@firebase/app-compat@0.2.31)(@firebase/app-types@0.9.1)(@firebase/app@0.10.1):
    '@firebase/firestore-compat': private
  /@firebase/firestore-types/3.0.1(@firebase/app-types@0.9.1)(@firebase/util@1.9.5):
    '@firebase/firestore-types': private
  /@firebase/firestore/4.6.0(@firebase/app@0.10.1):
    '@firebase/firestore': private
  /@firebase/functions-compat/0.3.10(@firebase/app-compat@0.2.31)(@firebase/app@0.10.1):
    '@firebase/functions-compat': private
  /@firebase/functions-types/0.6.1:
    '@firebase/functions-types': private
  /@firebase/functions/0.11.4(@firebase/app@0.10.1):
    '@firebase/functions': private
  /@firebase/installations-compat/0.2.6(@firebase/app-compat@0.2.31)(@firebase/app-types@0.9.1)(@firebase/app@0.10.1):
    '@firebase/installations-compat': private
  /@firebase/installations-types/0.5.1(@firebase/app-types@0.9.1):
    '@firebase/installations-types': private
  /@firebase/installations/0.6.6(@firebase/app@0.10.1):
    '@firebase/installations': private
  /@firebase/logger/0.4.1:
    '@firebase/logger': private
  /@firebase/messaging-compat/0.2.8(@firebase/app-compat@0.2.31)(@firebase/app@0.10.1):
    '@firebase/messaging-compat': private
  /@firebase/messaging-interop-types/0.2.1:
    '@firebase/messaging-interop-types': private
  /@firebase/messaging/0.12.8(@firebase/app@0.10.1):
    '@firebase/messaging': private
  /@firebase/performance-compat/0.2.6(@firebase/app-compat@0.2.31)(@firebase/app@0.10.1):
    '@firebase/performance-compat': private
  /@firebase/performance-types/0.2.1:
    '@firebase/performance-types': private
  /@firebase/performance/0.6.6(@firebase/app@0.10.1):
    '@firebase/performance': private
  /@firebase/remote-config-compat/0.2.6(@firebase/app-compat@0.2.31)(@firebase/app@0.10.1):
    '@firebase/remote-config-compat': private
  /@firebase/remote-config-types/0.3.1:
    '@firebase/remote-config-types': private
  /@firebase/remote-config/0.4.6(@firebase/app@0.10.1):
    '@firebase/remote-config': private
  /@firebase/storage-compat/0.3.7(@firebase/app-compat@0.2.31)(@firebase/app-types@0.9.1)(@firebase/app@0.10.1):
    '@firebase/storage-compat': private
  /@firebase/storage-types/0.8.1(@firebase/app-types@0.9.1)(@firebase/util@1.9.5):
    '@firebase/storage-types': private
  /@firebase/storage/0.12.4(@firebase/app@0.10.1):
    '@firebase/storage': private
  /@firebase/util/1.9.5:
    '@firebase/util': private
  /@firebase/webchannel-wrapper/0.10.6:
    '@firebase/webchannel-wrapper': private
  /@grpc/grpc-js/1.9.14:
    '@grpc/grpc-js': private
  /@grpc/proto-loader/0.7.12:
    '@grpc/proto-loader': private
  /@humanwhocodes/config-array/0.11.14:
    '@humanwhocodes/config-array': private
  /@humanwhocodes/module-importer/1.0.1:
    '@humanwhocodes/module-importer': private
  /@humanwhocodes/object-schema/2.0.3:
    '@humanwhocodes/object-schema': private
  /@jridgewell/gen-mapping/0.3.5:
    '@jridgewell/gen-mapping': private
  /@jridgewell/resolve-uri/3.1.2:
    '@jridgewell/resolve-uri': private
  /@jridgewell/set-array/1.2.1:
    '@jridgewell/set-array': private
  /@jridgewell/sourcemap-codec/1.4.15:
    '@jridgewell/sourcemap-codec': private
  /@jridgewell/trace-mapping/0.3.25:
    '@jridgewell/trace-mapping': private
  /@nodelib/fs.scandir/2.1.5:
    '@nodelib/fs.scandir': private
  /@nodelib/fs.stat/2.0.5:
    '@nodelib/fs.stat': private
  /@nodelib/fs.walk/1.2.8:
    '@nodelib/fs.walk': private
  /@protobufjs/aspromise/1.1.2:
    '@protobufjs/aspromise': private
  /@protobufjs/base64/1.1.2:
    '@protobufjs/base64': private
  /@protobufjs/codegen/2.0.4:
    '@protobufjs/codegen': private
  /@protobufjs/eventemitter/1.1.0:
    '@protobufjs/eventemitter': private
  /@protobufjs/fetch/1.1.0:
    '@protobufjs/fetch': private
  /@protobufjs/float/1.0.2:
    '@protobufjs/float': private
  /@protobufjs/inquire/1.1.0:
    '@protobufjs/inquire': private
  /@protobufjs/path/1.1.2:
    '@protobufjs/path': private
  /@protobufjs/pool/1.1.0:
    '@protobufjs/pool': private
  /@protobufjs/utf8/1.1.0:
    '@protobufjs/utf8': private
  /@remix-run/router/1.15.3:
    '@remix-run/router': private
  /@rollup/rollup-android-arm-eabi/4.14.2:
    '@rollup/rollup-android-arm-eabi': private
  /@rollup/rollup-android-arm64/4.14.2:
    '@rollup/rollup-android-arm64': private
  /@rollup/rollup-darwin-arm64/4.14.2:
    '@rollup/rollup-darwin-arm64': private
  /@rollup/rollup-darwin-x64/4.14.2:
    '@rollup/rollup-darwin-x64': private
  /@rollup/rollup-linux-arm-gnueabihf/4.14.2:
    '@rollup/rollup-linux-arm-gnueabihf': private
  /@rollup/rollup-linux-arm64-gnu/4.14.2:
    '@rollup/rollup-linux-arm64-gnu': private
  /@rollup/rollup-linux-arm64-musl/4.14.2:
    '@rollup/rollup-linux-arm64-musl': private
  /@rollup/rollup-linux-powerpc64le-gnu/4.14.2:
    '@rollup/rollup-linux-powerpc64le-gnu': private
  /@rollup/rollup-linux-riscv64-gnu/4.14.2:
    '@rollup/rollup-linux-riscv64-gnu': private
  /@rollup/rollup-linux-s390x-gnu/4.14.2:
    '@rollup/rollup-linux-s390x-gnu': private
  /@rollup/rollup-linux-x64-gnu/4.14.2:
    '@rollup/rollup-linux-x64-gnu': private
  /@rollup/rollup-linux-x64-musl/4.14.2:
    '@rollup/rollup-linux-x64-musl': private
  /@rollup/rollup-win32-arm64-msvc/4.14.2:
    '@rollup/rollup-win32-arm64-msvc': private
  /@rollup/rollup-win32-ia32-msvc/4.14.2:
    '@rollup/rollup-win32-ia32-msvc': private
  /@rollup/rollup-win32-x64-msvc/4.14.2:
    '@rollup/rollup-win32-x64-msvc': private
  /@types/babel__core/7.20.5:
    '@types/babel__core': private
  /@types/babel__generator/7.6.8:
    '@types/babel__generator': private
  /@types/babel__template/7.4.4:
    '@types/babel__template': private
  /@types/babel__traverse/7.20.5:
    '@types/babel__traverse': private
  /@types/estree/1.0.5:
    '@types/estree': private
  /@types/history/4.7.11:
    '@types/history': private
  /@types/json-schema/7.0.15:
    '@types/json-schema': private
  /@types/node/20.12.7:
    '@types/node': private
  /@types/prop-types/15.7.12:
    '@types/prop-types': private
  /@types/react-router/5.1.20:
    '@types/react-router': private
  /@types/semver/7.5.8:
    '@types/semver': private
  /@typescript-eslint/scope-manager/7.6.0:
    '@typescript-eslint/scope-manager': public
  /@typescript-eslint/type-utils/7.6.0(eslint@8.57.0)(typescript@5.4.5):
    '@typescript-eslint/type-utils': public
  /@typescript-eslint/types/7.6.0:
    '@typescript-eslint/types': public
  /@typescript-eslint/typescript-estree/7.6.0(typescript@5.4.5):
    '@typescript-eslint/typescript-estree': public
  /@typescript-eslint/utils/7.6.0(eslint@8.57.0)(typescript@5.4.5):
    '@typescript-eslint/utils': public
  /@typescript-eslint/visitor-keys/7.6.0:
    '@typescript-eslint/visitor-keys': public
  /@ungap/structured-clone/1.2.0:
    '@ungap/structured-clone': private
  /acorn-jsx/5.3.2(acorn@8.11.3):
    acorn-jsx: private
  /acorn/8.11.3:
    acorn: private
  /ajv/6.12.6:
    ajv: private
  /ansi-regex/5.0.1:
    ansi-regex: private
  /ansi-styles/4.3.0:
    ansi-styles: private
  /anymatch/3.1.3:
    anymatch: private
  /argparse/2.0.1:
    argparse: private
  /array-union/2.1.0:
    array-union: private
  /balanced-match/1.0.2:
    balanced-match: private
  /binary-extensions/2.3.0:
    binary-extensions: private
  /brace-expansion/1.1.11:
    brace-expansion: private
  /braces/3.0.2:
    braces: private
  /browserslist/4.23.0:
    browserslist: private
  /callsites/3.1.0:
    callsites: private
  /caniuse-lite/1.0.30001610:
    caniuse-lite: private
  /chalk/4.1.2:
    chalk: private
  /chokidar/3.6.0:
    chokidar: private
  /cliui/8.0.1:
    cliui: private
  /color-convert/2.0.1:
    color-convert: private
  /color-name/1.1.4:
    color-name: private
  /concat-map/0.0.1:
    concat-map: private
  /convert-source-map/2.0.0:
    convert-source-map: private
  /cross-spawn/7.0.3:
    cross-spawn: private
  /csstype/3.1.3:
    csstype: private
  /debug/4.3.4:
    debug: private
  /deep-is/0.1.4:
    deep-is: private
  /dir-glob/3.0.1:
    dir-glob: private
  /doctrine/3.0.0:
    doctrine: private
  /electron-to-chromium/1.4.736:
    electron-to-chromium: private
  /emoji-regex/8.0.0:
    emoji-regex: private
  /esbuild/0.20.2:
    esbuild: private
  /escalade/3.1.2:
    escalade: private
  /escape-string-regexp/4.0.0:
    escape-string-regexp: private
  /eslint-scope/7.2.2:
    eslint-scope: public
  /eslint-visitor-keys/3.4.3:
    eslint-visitor-keys: public
  /espree/9.6.1:
    espree: private
  /esquery/1.5.0:
    esquery: private
  /esrecurse/4.3.0:
    esrecurse: private
  /estraverse/5.3.0:
    estraverse: private
  /esutils/2.0.3:
    esutils: private
  /fast-deep-equal/3.1.3:
    fast-deep-equal: private
  /fast-glob/3.3.2:
    fast-glob: private
  /fast-json-stable-stringify/2.1.0:
    fast-json-stable-stringify: private
  /fast-levenshtein/2.0.6:
    fast-levenshtein: private
  /fastq/1.17.1:
    fastq: private
  /faye-websocket/0.11.4:
    faye-websocket: private
  /file-entry-cache/6.0.1:
    file-entry-cache: private
  /fill-range/7.0.1:
    fill-range: private
  /find-up/5.0.0:
    find-up: private
  /flat-cache/3.2.0:
    flat-cache: private
  /flatted/3.3.1:
    flatted: private
  /fs.realpath/1.0.0:
    fs.realpath: private
  /fsevents/2.3.3:
    fsevents: private
  /gensync/1.0.0-beta.2:
    gensync: private
  /get-caller-file/2.0.5:
    get-caller-file: private
  /glob-parent/6.0.2:
    glob-parent: private
  /glob/7.2.3:
    glob: private
  /globals/13.24.0:
    globals: private
  /globby/11.1.0:
    globby: private
  /graphemer/1.4.0:
    graphemer: private
  /has-flag/4.0.0:
    has-flag: private
  /http-parser-js/0.5.8:
    http-parser-js: private
  /idb/7.1.1:
    idb: private
  /ignore/5.3.1:
    ignore: private
  /immutable/4.3.5:
    immutable: private
  /import-fresh/3.3.0:
    import-fresh: private
  /imurmurhash/0.1.4:
    imurmurhash: private
  /inflight/1.0.6:
    inflight: private
  /inherits/2.0.4:
    inherits: private
  /is-binary-path/2.1.0:
    is-binary-path: private
  /is-extglob/2.1.1:
    is-extglob: private
  /is-fullwidth-code-point/3.0.0:
    is-fullwidth-code-point: private
  /is-glob/4.0.3:
    is-glob: private
  /is-number/7.0.0:
    is-number: private
  /is-path-inside/3.0.3:
    is-path-inside: private
  /isexe/2.0.0:
    isexe: private
  /js-tokens/4.0.0:
    js-tokens: private
  /js-yaml/4.1.0:
    js-yaml: private
  /jsesc/2.5.2:
    jsesc: private
  /json-buffer/3.0.1:
    json-buffer: private
  /json-schema-traverse/0.4.1:
    json-schema-traverse: private
  /json-stable-stringify-without-jsonify/1.0.1:
    json-stable-stringify-without-jsonify: private
  /json5/2.2.3:
    json5: private
  /keyv/4.5.4:
    keyv: private
  /levn/0.4.1:
    levn: private
  /locate-path/6.0.0:
    locate-path: private
  /lodash.camelcase/4.3.0:
    lodash.camelcase: private
  /lodash.merge/4.6.2:
    lodash.merge: private
  /long/5.2.3:
    long: private
  /loose-envify/1.4.0:
    loose-envify: private
  /lru-cache/6.0.0:
    lru-cache: private
  /merge2/1.4.1:
    merge2: private
  /micromatch/4.0.5:
    micromatch: private
  /minimatch/3.1.2:
    minimatch: private
  /ms/2.1.2:
    ms: private
  /nanoid/3.3.7:
    nanoid: private
  /natural-compare/1.4.0:
    natural-compare: private
  /node-releases/2.0.14:
    node-releases: private
  /normalize-path/3.0.0:
    normalize-path: private
  /once/1.4.0:
    once: private
  /optionator/0.9.3:
    optionator: private
  /p-limit/3.1.0:
    p-limit: private
  /p-locate/5.0.0:
    p-locate: private
  /parent-module/1.0.1:
    parent-module: private
  /path-exists/4.0.0:
    path-exists: private
  /path-is-absolute/1.0.1:
    path-is-absolute: private
  /path-key/3.1.1:
    path-key: private
  /path-type/4.0.0:
    path-type: private
  /picocolors/1.0.0:
    picocolors: private
  /picomatch/2.3.1:
    picomatch: private
  /postcss/8.4.38:
    postcss: private
  /prelude-ls/1.2.1:
    prelude-ls: private
  /protobufjs/7.2.6:
    protobufjs: private
  /punycode/2.3.1:
    punycode: private
  /queue-microtask/1.2.3:
    queue-microtask: private
  /react-refresh/0.14.0:
    react-refresh: private
  /react-router/6.22.3(react@18.2.0):
    react-router: private
  /readdirp/3.6.0:
    readdirp: private
  /require-directory/2.1.1:
    require-directory: private
  /resolve-from/4.0.0:
    resolve-from: private
  /reusify/1.0.4:
    reusify: private
  /rimraf/3.0.2:
    rimraf: private
  /rollup/4.14.2:
    rollup: private
  /run-parallel/1.2.0:
    run-parallel: private
  /safe-buffer/5.2.1:
    safe-buffer: private
  /scheduler/0.23.0:
    scheduler: private
  /semver/7.6.0:
    semver: private
  /shebang-command/2.0.0:
    shebang-command: private
  /shebang-regex/3.0.0:
    shebang-regex: private
  /slash/3.0.0:
    slash: private
  /source-map-js/1.2.0:
    source-map-js: private
  /string-width/4.2.3:
    string-width: private
  /strip-ansi/6.0.1:
    strip-ansi: private
  /strip-json-comments/3.1.1:
    strip-json-comments: private
  /supports-color/7.2.0:
    supports-color: private
  /text-table/0.2.0:
    text-table: private
  /to-fast-properties/2.0.0:
    to-fast-properties: private
  /to-regex-range/5.0.1:
    to-regex-range: private
  /ts-api-utils/1.3.0(typescript@5.4.5):
    ts-api-utils: private
  /tslib/2.6.2:
    tslib: private
  /type-check/0.4.0:
    type-check: private
  /type-fest/0.20.2:
    type-fest: private
  /undici-types/5.26.5:
    undici-types: private
  /undici/5.28.4:
    undici: private
  /update-browserslist-db/1.0.13(browserslist@4.23.0):
    update-browserslist-db: private
  /uri-js/4.4.1:
    uri-js: private
  /websocket-driver/0.7.4:
    websocket-driver: private
  /websocket-extensions/0.1.4:
    websocket-extensions: private
  /which/2.0.2:
    which: private
  /wrap-ansi/7.0.0:
    wrap-ansi: private
  /wrappy/1.0.2:
    wrappy: private
  /y18n/5.0.8:
    y18n: private
  /yallist/4.0.0:
    yallist: private
  /yargs-parser/21.1.1:
    yargs-parser: private
  /yargs/17.7.2:
    yargs: private
  /yocto-queue/0.1.0:
    yocto-queue: private
included:
  dependencies: true
  devDependencies: true
  optionalDependencies: true
injectedDeps: {}
layoutVersion: 5
nodeLinker: isolated
packageManager: pnpm@8.15.5
pendingBuilds: []
prunedAt: Mon, 15 Apr 2024 04:27:17 GMT
publicHoistPattern:
  - '*eslint*'
  - '*prettier*'
registries:
  default: https://registry.npmjs.org/
skipped:
  - /@esbuild/aix-ppc64/0.20.2
  - /@esbuild/android-arm/0.20.2
  - /@esbuild/android-arm64/0.20.2
  - /@esbuild/android-x64/0.20.2
  - /@esbuild/darwin-x64/0.20.2
  - /@esbuild/freebsd-arm64/0.20.2
  - /@esbuild/freebsd-x64/0.20.2
  - /@esbuild/linux-arm/0.20.2
  - /@esbuild/linux-arm64/0.20.2
  - /@esbuild/linux-ia32/0.20.2
  - /@esbuild/linux-loong64/0.20.2
  - /@esbuild/linux-mips64el/0.20.2
  - /@esbuild/linux-ppc64/0.20.2
  - /@esbuild/linux-riscv64/0.20.2
  - /@esbuild/linux-s390x/0.20.2
  - /@esbuild/linux-x64/0.20.2
  - /@esbuild/netbsd-x64/0.20.2
  - /@esbuild/openbsd-x64/0.20.2
  - /@esbuild/sunos-x64/0.20.2
  - /@esbuild/win32-arm64/0.20.2
  - /@esbuild/win32-ia32/0.20.2
  - /@esbuild/win32-x64/0.20.2
  - /@rollup/rollup-android-arm-eabi/4.14.2
  - /@rollup/rollup-android-arm64/4.14.2
  - /@rollup/rollup-darwin-x64/4.14.2
  - /@rollup/rollup-linux-arm-gnueabihf/4.14.2
  - /@rollup/rollup-linux-arm64-gnu/4.14.2
  - /@rollup/rollup-linux-arm64-musl/4.14.2
  - /@rollup/rollup-linux-powerpc64le-gnu/4.14.2
  - /@rollup/rollup-linux-riscv64-gnu/4.14.2
  - /@rollup/rollup-linux-s390x-gnu/4.14.2
  - /@rollup/rollup-linux-x64-gnu/4.14.2
  - /@rollup/rollup-linux-x64-musl/4.14.2
  - /@rollup/rollup-win32-arm64-msvc/4.14.2
  - /@rollup/rollup-win32-ia32-msvc/4.14.2
  - /@rollup/rollup-win32-x64-msvc/4.14.2
storeDir: /Users/matthewsong/Library/pnpm/store/v3
virtualStoreDir: .pnpm
